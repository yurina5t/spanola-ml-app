...
async def signup(data: UserCreate, session: Session = Depends(get_session)) -> Dict[str, str]:
    ...
    try:
        ...
        return {"message": "Пользователь успешно зарегистрирован и кошелёк создан", "user_id": str(user.id)}
...
async def signin(data: UserLogin, session: Session = Depends(get_session)) -> Dict[str, str]:
    """
    ...
    return {"message": "Вход выполнен успешно", "user_id": str(user.id)}
...
async def get_all_users(session: Session = Depends(get_session)) -> List[UserResponse]:
    """
...
async def delete_user(user_id: int, session: Session = Depends(get_session)) -> Dict[str, str]:
    """
    ...
    try:
        ...
        return {"message": "Пользователь удалён", "user_id": str(user_id)}
    except HTTPException:
        raise
    except Exception as e:
