...
@tasklog_route.get("/history/{user_id}", response_model=List[TaskLogItem],
                   summary="История задач пользователя")
def tasks_history(user_id: int, limit: int = 100, session: Session = Depends(get_session)) -> List[TaskLogItem]:
    """
...
@tasklog_route.get("/{task_id}", response_model=TaskLogItem, summary="Детали лога задачи")
